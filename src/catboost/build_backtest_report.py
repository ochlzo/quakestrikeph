"""Build a comprehensive, self-contained HTML report for a CatBoost backtest.
...
The report is single-family (CatBoost) and oriented around the Path B story:
...
Usage:
    ...
    python src/catboost/build_backtest_report.py \
        --backtest-dir src/outputs/catboost/backtests_mc_1_0 \
        --output src/docs/catboost_backtest_report.html
"""
...
<title>CatBoost Aftershock Backtest &mdash; {year_label}</title>
...
<body>
<header>
<h1>CatBoost Aftershock Backtest &mdash; {year_label}</h1>
...
def parse_args():
    ...
    parser.add_argument(
        "--backtest-dir",
        type=Path,
        default=Path("src/outputs/catboost/backtests_mc_1_0"),
        help="Directory holding backtest_metrics.json (and optionally backtest_predictions.csv).",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("src/docs/catboost_backtest_report.html"),
    )
