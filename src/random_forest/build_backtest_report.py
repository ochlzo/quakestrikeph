"""Build a comprehensive, self-contained HTML report for a Random Forest backtest.
...
The report is single-family (Random Forest) and oriented around the Path B story:
...
Usage:
    ...
    python src/random_forest/build_backtest_report.py \
        --backtest-dir src/outputs/random-forest/backtests_mc_1_0 \
        --output src/docs/random-forest_backtest_report.html
"""
...
<title>Random Forest Aftershock Backtest &mdash; {year_label}</title>
...
<body>
<header>
<h1>Random Forest Aftershock Backtest &mdash; {year_label}</h1>
...
def parse_args():
    ...
    parser.add_argument(
        "--backtest-dir",
        type=Path,
        default=Path("src/outputs/random-forest/backtests_mc_1_0"),
        help="Directory holding backtest_metrics.json (and optionally backtest_predictions.csv).",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("src/docs/random-forest_backtest_report.html"),
    )
