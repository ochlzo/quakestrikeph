"""Build a comprehensive, self-contained HTML report for a LightGBM backtest.
...
The report is single-family (LightGBM) and oriented around the Path B story:
...
Usage:
    ...
    python src/lightgbm/build_backtest_report.py \
        --backtest-dir src/outputs/lightgbm/backtests_mc_1_0 \
        --output src/docs/lightgbm_backtest_report.html
"""
...
<title>LightGBM Aftershock Backtest &mdash; {year_label}</title>
...
<body>
<header>
<h1>LightGBM Aftershock Backtest &mdash; {year_label}</h1>
...
def parse_args():
    ...
    parser.add_argument(
        "--backtest-dir",
        type=Path,
        default=Path("src/outputs/lightgbm/backtests_mc_1_0"),
        help="Directory holding backtest_metrics.json (and optionally backtest_predictions.csv).",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("src/docs/lightgbm_backtest_report.html"),
    )
